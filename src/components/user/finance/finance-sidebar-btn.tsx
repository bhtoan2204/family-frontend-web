...
interface FinanceSidebarBtnProps {
    name: string;
    description: string;
    itemId: number;
    isSelected?: boolean;
...
const FinanceSidebarBtn = ({
    name,
    description,
    itemId,
    isSelected,
}: FinanceSidebarBtnProps) => {
    const params = useParams();
    const router = useRouter();
    const onClick = () => {
        router.push(
            `/family/${params?.familyId}/finance/${itemId}`
        );
    };
    ...
    return (
        <button
            ...
            className={cn(
                ...
                params?.financeCode === itemId.toString() &&
...
export default FinanceSidebarBtn;
